...
export class AuthController {
  ...
  @Post('logout') @UseGuards(AccessJwtGuard)
  async logout(@Req() req: { user: ReqUser }) { return this.auth.logout(req.user.sub); }

  @Post('refresh') @UseGuards(RefreshJwtGuard)
  ...
  @Delete('delete-account') @UseGuards(AccessJwtGuard)
  deleteAccount(@Req() req: { user: ReqUser }) { return this.auth.deleteAccount(req.user.sub); }

  @Post('request-password-reset') async requestReset(@Body() b: any) {
