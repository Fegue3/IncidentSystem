...
describe('Integrations (e2e)', () => {
  ...
  it('returns defaults for a new user (ON)', async () => {
    const ctx = await bootstrapE2E();
    await resetDb(ctx.prisma);
    ...
    expect(res.body.datadog.notificationsEnabled).toBe(true);
    expect(res.body.pagerduty.notificationsEnabled).toBe(true);
    expect(res.body.discord.notificationsEnabled).toBe(true);

    expect(res.body.datadog.lastSavedAt).toBeNull();
    expect(res.body.pagerduty.lastSavedAt).toBeNull();
    expect(res.body.discord.lastSavedAt).toBeNull();
  ...
  it('PATCH toggles and persists per-user (Datadog)', async () => {
    ...
    expect(patch.body.datadog.notificationsEnabled).toBe(false);
    ...
    expect(res2.body.datadog.notificationsEnabled).toBe(false);
  ...
  it('settings are isolated between users', async () => {
    ...
    expect(resU1.body.pagerduty.notificationsEnabled).toBe(false);
    expect(resU2.body.pagerduty.notificationsEnabled).toBe(true);
